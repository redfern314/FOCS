...
let print_term ppf t = 
  ...
  and app t u = 
    ...
    | Var x -> x^" "^(arg u)
    | _ -> "("^(term t)^") "^(arg u)   in
  Format.fprintf ppf "%s" (term t)
(* Hack to add a printer to the OCaml shell to print lambda terms *)
...
let setinter s t = 
  List.filter (fun x -> (List.mem x t)) s
...
let rec noCapture t bound = begin
  (* (Printf.printf "Term: ");
  (show_term t);
  (Printf.printf "Bound: ");
  List.iter (Printf.printf "%s ") bound;
  (Printf.printf "\n"); *)
  match t with
    | Var x -> not (List.mem x bound)
    | Lam (x,y) -> (setinter (fv t) bound)=[]
    | App (t,u) -> (noCapture t bound) && (noCapture u bound)
  end
;;

let rec subst_rec t1 v t2 bound = begin
  (* (Printf.printf "Term: ");
  (show_term t1);
  (Printf.printf "Free: ");
  ...
  (Printf.printf "\n"); *)
  match t1 with
  | Var x -> if (x=v && (not (List.mem x bound)) && (noCapture t2 bound)) then begin (* (Printf.printf "replace\n"); *) t2 end else Var x
  | Lam (x,t) -> Lam (x,(subst_rec t v t2 ([x]@bound)))
  | App (t,u) -> App ((subst_rec t v t2 bound),(subst_rec u v t2 bound))
end
;;

let subst t1 v t2 = (subst_rec t1 v t2 []);;
...
let rec has_redex t = 
  match t with
    ...
    | Lam (x,y) -> (has_redex y)
    | App (Lam _, _) -> true
    | App (x,y) -> ((has_redex x) || (has_redex y))
;;

let rec reduce_once t = 
  match t with
    | Var _ -> t
    | Lam (x,y) -> Lam (x,(reduce_once y))
    | App (Lam (x,y), v) -> (subst y x v)
    | App (x,y) -> if (has_redex x) then App ((reduce_once x),y)
                    else if (has_redex y) then App (x,(reduce_once y))
                    else t
;;

let rec reduce t = begin
  (* show_term t; *)
  if (has_redex t) then (reduce (reduce_once t)) else t
end
...
let string_of_mod_state string_of_state s =
  match s with
    | Accept -> "Accept"
    | Reject -> "Reject"
    | Rewind x -> "Rewind:"^x
    | State (x,y) -> x^":"^(string_of_state y)
...
let make_states_module (name,m,_) = (Rewind name)::(List.map (fun x -> (State (name,x))) m.states_M)

let make_states m = Accept::(Reject::(List.flatten (List.map (fun x -> (make_states_module x)) m.modules)))

let find_whole_module name m = 
  let res = 
    (List.fold_right 
      (fun (x,m,t) r -> if x=name then Some (name,m,t) else r) 
      m.modules None)
    in
  match res with
    None -> fail "What are you doing with your life? That module isn't here."
    | Some found -> found
;;

let find_module name m = 
  let (_,tm,_) = (find_whole_module name m) in tm
;;

let make_delta m =
  let delta (q,a) = 
    match q,a with
      | Accept,_ -> (Accept,a,Stay)
      | Reject,_ -> (Reject,a,Stay)
      | State (x,y),_ ->
        let (_,thisMod,(acctar,rejtar)) = (find_whole_module x m) in
          if (y=thisMod.accept_M) then
            match acctar with
              | ToStart nextName -> (Rewind nextName,a,Stay) 
              | ToAccept -> (Accept,a,Stay)
              | ToReject -> (Reject,a,Stay)
          else if (y=thisMod.reject_M) then
            match rejtar with
              | ToStart nextName -> (Rewind nextName,a,Stay)
              | ToAccept -> (Accept,a,Stay)
              | ToReject -> (Reject,a,Stay)
          else 
            let (newState,newSym,dir) = (thisMod.delta_M (y,a)) in ((State (x,newState)),newSym,dir)
      | Rewind s,sym when sym = m.leftmost_M -> 
        let nextMod = (find_module s m) in
          (State (s,nextMod.start_M),a,Stay)
      | Rewind s,_ -> (Rewind s,a,Left)
    in delta
;;

let get_start_state m =
  let start_mod = (find_module m.start_module m) in
    (State (m.start_module,start_mod.start_M))
;;

let build_M string_of_state m =
  let delta = (make_delta m) in
    let states = (make_states m) in
      let start = (get_start_state m) in
        let machine_description = 
          { states = states;
            ...
            accept = Accept;
            reject = Reject } in
              (TM.build (string_of_mod_state string_of_state) machine_description)
;;
